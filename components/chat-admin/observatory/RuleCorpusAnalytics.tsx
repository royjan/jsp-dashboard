...
import { Loader2, Package, AlertTriangle, Database } from 'lucide-react'
import {
...
function KpiTile({ icon, label, value, sub }: { icon: React.ReactNode; label: string; value: any; sub?: string }) {
  return (
    <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-3">
...
export default function RuleCorpusAnalytics() {
  ...
  useEffect(() => {
    fetch('/api/flow-decisions/stats').then((r) => r.json())
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <KpiTile icon={<AlertTriangle size={12} />} label="כיסוי embeddings" value={`${embPct}%`} sub={`${embMiss} חסרים`} />
