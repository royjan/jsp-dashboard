 ...
 *
 * REALITY: true cost / purchase price is NOT reliably available in Neon — it
 * lives in FINAPI (7ITP). dashboard.monthly_sales and dashboard.item_snapshots
 * have NO cost column. So margin is computed *defensively*: estimateMargin()
 * returns null whenever no cost is known, and the page renders a clear
 * "cost pending (FINAPI)" state. Revenue / quantity / avg sell price are real
 * and useful today.
 *
 * If a cost column is wired into the snapshot later, add it to the SELECT below
 * and to estimateMargin() — nothing else needs to change.
 */
  ...
  try {
    ...
    const byItem = byItemResult.rows.map((r: any) => {
      ...
      const row: MarginRow = {
        ...
        quantity,
        avg_price,
        cost: null, // no cost in Neon — see header note
      }
      return { ...row, margin_pct: estimateMargin(row) }
    })
    ...
    const byCategory = byCategoryResult.rows.map((r: any) => {
      ...
      const row: MarginRow = { category: r.category, revenue, quantity, avg_price, cost: null }
      return {
        ...
        margin_pct: estimateMargin(row),
      }
    })

    const s = summaryResult.rows[0] || {}
    const summaryRow: MarginRow = {
    ...
    const response = {
      summary: {
        ...
        // null until cost is wired from FINAPI — page shows "pending" state.
        est_gross_margin_pct: estimateMargin(summaryRow),
      },
      byCategory,
      byItem,
      // Surface the cost situation to the client so it can render the right state.
      cost_available: byItem.some((r: any) => r.margin_pct != null),
      note_he: 'מרווח גולמי בהמתנה — מחיר עלות נמצא ב-FINAPI (7ITP) ועדיין לא מחובר. הכנסה, כמות ומחיר מכירה ממוצע הם נתוני אמת.',
      note_en: 'Gross margin pending — cost price lives in FINAPI (7ITP) and is not wired yet. Revenue, quantity and avg sell price are real.',
